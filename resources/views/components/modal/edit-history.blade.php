@props(['history'])
<div x-show="openEditHistory{{ $history->id }}" x-transition:enter="transition duration-300 ease-out"
            ...
            <form action="{{ route('history.update', $history->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="flex flex-col gap-5">
                    <div class="w-full flex flex-col gap-1">
                        ...
                        <select name="status" id="status" class="w-full rounded-lg" required>
                            <option value="" selected disabled>Pilih Status</option>
                            <option {{ $history->status == 'pending' ? 'selected' : '' }} value="pending">pending
                            </option>
                            <option {{ $history->status == 'disetujui' ? 'selected' : '' }} value="disetujui">disetujui
                            </option>
                            <option {{ $history->status == 'sedang dipinjam' ? 'selected' : '' }}
                                value="sedang dipinjam">sedang
                                dipinjam</option>
                            <option {{ $history->status == 'dikembalikan' ? 'selected' : '' }} value="dikembalikan">
                                dikembalikan</option>
                        </select>
                    </div>
                    <div class="w-full">
                        <div class="relative z-0 w-full mb-5 group">
                            <input type="date" name="return_date" id="return_date"
                                ...
                                placeholder=" " value="{{ $history->return_date }}" />
                ...
                <div class="mt-5 flex items-center justify-between">
                    <button @click="openEditHistory{{ $history->id }} = false" type="button"
