@props(['book'])
<div x-show="openEditBook{{ $book->id }}" x-transition:enter="transition duration-300 ease-out"
            ...
            <form action="{{ route('book.update', $book->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="flex gap-5">
                    <div class="flex justify-center w-40 flex-none">
                        <div class="relative">
                            <img id="picture{{ $book->id }}" class="object-cover w-40 h-40 rounded-lg"
                            ...
                            <div class="absolute z-10 top-0  opacity-0 hover:opacity-100 rounded-lg">
                                <label for="pictureInput{{ $book->id }}">
                                    ...
                                    <input accept="image/*" type="file" name="picture" class="hidden"
                                        id="pictureInput{{ $book->id }}" />
                                </label>
                            </div>
                            <script>
                                pictureInput{{ $book->id }}.onchange = evt => {
                                    const [file] = pictureInput{{ $book->id }}.files
                                    if (file) {
                                        picture{{ $book->id }}.src = URL.createObjectURL(file)
                                    }
                                }
                            </script>
                        </div>
                    ...
                    <div class="w-full">
                        ...
                        <div class="relative z-0 w-full mb-5 group">
                            <input type="number" name="release" id="release"
                                class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                                placeholder=" " value="{{ $book->release }}" required />
                            <label for="release"
                                class="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Book
                                Release</label>
                        </div>
                        <div class="relative z-0 w-full mb-5 group">
                            <input type="text" name="publisher" id="publisher"
                                class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                                placeholder=" " value="{{ $book->publisher }}" required />
                            <label for="publisher"
                                class="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Book
                                Publisher</label>
                        </div>
                        <div class="relative z-0 w-full mb-5 group">
                            <input type="number" name="copy" id="copy"
                                class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                                placeholder=" " value="{{ $book->copy }}" required />
                            <label for="copy"
                ...
                <div class="mt-5 flex items-center justify-between">
                    <button @click="openEditBook{{ $book->id }} = false" type="button"
