        ...
        <div class="flex flex-col md:flex-row md:justify-between md:items-center">
            <div class="flex items-center justify-between">
                <div class="flex items-center">
                    <a href="{{ route('welcome') }}" class="flex items-center justify-center gap-3">
                        <img class="w-auto h-6 sm:h-7" src="{{ asset('assets/images/logo.png') }}" alt="">
                        <p>E-PERPUS SMK MERDEKA</p>
                    </a>
            ...
            <div x-cloak :class="[isOpen ? 'translate-x-0 opacity-100 ' : 'opacity-0 -translate-x-full']"
                ...
                <div class="flex flex-col md:flex-row md:mx-1">
                    <a href="{{ route('pinjaman-saya') }}">Pinjaman Saya</a>
                    <form action="{{ route('logout') }}" method="POST" class="cursor-pointer">
                        @csrf
                        <a class="flex items-center px-4 text-gray-600 transition-colors duration-300 transform rounded-lg "
                            :href="route('logout')"
        ...
        <div class="py-3 mt-3 -mx-3 overflow-y-auto whitespace-nowrap scroll-hidden">
            @foreach (\App\Models\category::all() as $item)
                <a class="mx-4 text-sm leading-5 text-gray-700 transition-colors duration-300 transform dark:text-gray-200 hover:text-blue-600 dark:hover:text-blue-400 hover:underline md:my-0"
                    href="{{ route('category.show', $item->id) }}">{{ $item->category_name }}</a>
            @endforeach
        </div>
    </div>
</nav>
